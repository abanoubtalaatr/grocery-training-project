<aside
    class="fixed top-0 {{ app()->getLocale() === 'ar' ? 'right-0' : 'left-0' }}
           h-screen w-64 bg-gray-950 text-gray-300 flex flex-col z-50">
    {{-- Brand --}}
    <div class="px-6 py-5 border-b border-gray-800 flex items-center gap-3">
        ...
        <span class="font-semibold text-white text-base tracking-tight">Grocery Admin</span>
    </div>
        ...
        <ul class="space-y-0.5">
            ...
            <li>
                <a href="{{ route('admin.categories.index') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150
                        {{ request()->routeIs('admin.categories.*') ? 'bg-emerald-500/10 text-emerald-400' : 'text-gray-400 hover:bg-gray-800 hover:text-white' }}">
                    <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <li>
                <a href="#"
                    ...
                    <span
                        class="ml-auto bg-emerald-500/20 text-emerald-400 text-[10px] font-semibold px-2 py-0.5 rounded-full">
                        12
                    </span>
                </a>
            </li>
    ...
    <div class="px-4 py-4 border-t border-gray-800">
        <div class="flex items-center gap-3">
            <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=10b981&color=fff"
                class="w-8 h-8 rounded-full flex-shrink-0" alt="{{ Auth::user()->name }}">
            <div class="min-w-0">
            ...
            <form method="POST" action="{{ route('admin.logout') }}" class="ml-auto">
                @csrf
                <button type="submit" class="text-gray-500 hover:text-gray-300 transition-colors">
